...
namespace blgz {
    ...
    std::vector<Vertex> MakeGeometry(const sdf::Geometry &geom) {
        // clang-format off
        static constexpr std::vector<Vertex>
        (*MakeGeometryFunctions[])(const sdf::Geometry &) = {
            nullptr,      // EMPTY     = 0
            MakeBox,      // BOX       = 1
            MakeCylinder, // CYLINDER  = 2
            nullptr,      // PLANE     = 3
            MakeSphere,   // SPHERE    = 4
            MakeMesh,     // MESH      = 5
            nullptr,      // HEIGHTMAP = 6
        ...
        // clang-format on
        const int type = static_cast<int>(geom.Type());
        return (0 <= type && type < std::size(MakeGeometryFunctions) && MakeGeometryFunctions[type])
                   ? MakeGeometryFunctions[type](geom)
                   : std::vector<Vertex>{};
    }
} // namespace blgz
